...
# guarantees Postgres + Keycloak are stopped and their volumes purged after
# the build is done, no matter how it ended.
#
# Usage:
#   bash scripts/graalvm-pgo/pgo-build.sh install               # full build + load tests + install to local repo
#   bash scripts/graalvm-pgo/pgo-build.sh verify -DskipTests    # what CI runs (deploy.yml)
#
# The pom.xml's compose-down execution in post-integration-test handles the
# happy-path teardown when mvn is run directly (no wrapper). This wrapper
# adds the safety net for failure / interruption / kill paths.
#
